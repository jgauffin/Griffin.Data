...
public static class ListExtensions
{
    ...
    /// </param>
    /// <returns>Found entities.</returns>
    public static async Task<List<TEntity>> List<TEntity>(this Session session, string? query = null,
        object? constraints = null) where TEntity : notnull
    {
        var entities = await session.List<TEntity>(new QueryOptions<TEntity>(query, constraints));
        foreach (var entity in entities) session.Track(entity);
